...
// Export all comments as PDF
const exportCommentsToPDF = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (postId) {
      whereClause.postId = postId as string;
    }
    ...
    const filename = `comments_export_${new Date().toISOString().split('T')[0]}.pdf`;
    ...
    // Add title
    doc.fontSize(20).font('Helvetica-Bold').text('Comments Export Report', { align: 'center' });
    doc.moveDown();
    doc.fontSize(12).font('Helvetica').text(`Generated: ${new Date().toLocaleString()}`, { align: 'center' });
    doc.fontSize(10).text(`Total Comments: ${comments.length}`, { align: 'center' });
    doc.moveDown(2);

    // Add statistics summary
    const stats = {
      positive: comments.filter(c => c.sentiment?.toLowerCase() === 'positive').length,
      negative: comments.filter(c => c.sentiment?.toLowerCase() === 'negative').length,
      neutral: comments.filter(c => c.sentiment?.toLowerCase() === 'neutral').length
    };

    doc.fontSize(14).font('Helvetica-Bold').text('Sentiment Summary', { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(11).font('Helvetica');
    doc.text(`• Positive: ${stats.positive} (${((stats.positive / comments.length) * 100).toFixed(1)}%)`);
    doc.text(`• Negative: ${stats.negative} (${((stats.negative / comments.length) * 100).toFixed(1)}%)`);
    doc.text(`• Neutral: ${stats.neutral} (${((stats.neutral / comments.length) * 100).toFixed(1)}%)`);
    doc.moveDown(2);

    // Add comments
    doc.fontSize(14).font('Helvetica-Bold').text('Comments Details', { underline: true });
    doc.moveDown(1);
    ...
    comments.forEach((comment, index) => {
      ...
      doc.fontSize(11).font('Helvetica-Bold')
        .text(`${index + 1}. ${comment.stakeholderName || 'Anonymous'}`, { continued: false });
      
      // Company and category info
      if (comment.company) {
        doc.fontSize(9).font('Helvetica')
          .fillColor('#666666')
          .text(`   Company: ${comment.company.name}`, { continued: true })
          .text(` | State: ${comment.company.state || 'N/A'}`, { continued: true })
          .text(` | Category: ${comment.company.businessCategory?.name || 'N/A'}`);
      } else {
        doc.fontSize(9).font('Helvetica')
          .fillColor('#666666')
          .text(`   Category: ${comment.businessCategory?.name || 'N/A'}`);
      }
      ...
      const sentimentColor = 
        comment.sentiment?.toLowerCase() === 'positive' ? '#00AA00' :
        comment.sentiment?.toLowerCase() === 'negative' ? '#CC0000' : '#888888';
      
      doc.fillColor(sentimentColor)
        .text(`   Sentiment: ${comment.sentiment || 'Unknown'}`, { continued: true })
        .fillColor('#666666')
        .text(` | Language: ${comment.language || 'N/A'}`, { continued: true })
        .text(` | Type: ${comment.commentType || 'N/A'}`);

      // Comment text
      doc.fontSize(10).fillColor('#000000').font('Helvetica');
      const commentText = comment.standardComment || comment.rawComment || 'No comment text';
      ...
      if (comment.keywords && comment.keywords.length > 0) {
        doc.fontSize(9).fillColor('#666666')
          .text(`   Keywords: ${comment.keywords.slice(0, 10).join(', ')}`);
      }

      // Summary if available
      if (comment.summary) {
        doc.fontSize(9).fillColor('#666666')
          .text(`   Summary: ${comment.summary.substring(0, 200)}${comment.summary.length > 200 ? '...' : ''}`);
      }

      // Timestamp
      doc.fontSize(8).fillColor('#999999')
        .text(`   Created: ${new Date(comment.createdAt).toLocaleString()}`);
    ...
    doc.fontSize(8).fillColor('#999999')
      .text(`End of Report - ${comments.length} comments exported`, 
        50, doc.page.height - 50, { align: 'center' });

    // Finalize PDF
    doc.end();

    console.log(`✅ Exported ${comments.length} comments to PDF`);

  } catch (error) {
    console.error("Error exporting comments to PDF:", error);
    throw new ApiError(500, "Failed to export comments to PDF");
  }
});
